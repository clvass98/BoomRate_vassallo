...
sndata_root = script_dir+'/SNANA'   # local SNANA files (filters, templates) inside the repo
model_path = script_dir+'/templates/non1a'
...
def run(redshift, baseline, sens, type=['iip'], dstep=3, dmstep=0.5, dastep=0.5,
    ...
    if 'ia' in type:
        ...
        for sdss_filter in glob.glob(script_dir+'/templates/filters/SDSS/?.dat'):
    ...
    if not os.path.isfile(model_pkl):
        ...
        if 'ia' in type:
            ...
            model_path = sndata_root+'/snsed'  # for Ia runs (not copied locally; add if you need Ia)
        if 'slsn' in type:
    ...
    if redshift > 1.5:
        vega_spec = loadtxt(script_dir+'/templates/vega_model.dat')
    else:
        vega_spec = loadtxt(script_dir+'/templates/vega_model_mod.dat')
...
def read_lc_model(model):
    ...
    for line in lines:
        if line.startswith('FILTER'):
            ...
            filter_path = filter_path.replace('$SNDATA_ROOT/filters',script_dir+'/templates/filters')
...
def rest_frame_Ia_lightcurve(dstep=3, verbose=True):
    models_dir = sndata_root+'/models/mlcs2k2/mlcs2k2.v007/'  # for Ia runs (not copied locally; add if you need Ia)
...
def ext_dist_Ia(ext,observed_filter,redshift,passskiprow,passwavemult):
    ...
    Jha = loadtxt(script_dir+'/templates/Jha_ext.txt')
    Jha[:,0] = Jha[:,0]/A_1*A_2
    p0 = [1.,1.]
...
def ext_dist_ccsn_old(ext,observed_filter,redshift,passskiprow, passwavemult,obs_extin='nominal',observed=False):
    ...
    if observed:
        if not os.path.isfile(script_dir+'/templates/ext_model.pkl'):
            f = open(script_dir+'/templates/ext_model.txt','r')
            lines = f.readlines()
            f.close()
            Av=[]
            for line in lines:
            ...
            Av=array(Av)
            pickle.dump(Av,open(script_dir+'/templates/ext_model.pkl','wb'))
        else:
            Av = pickle.load(open(script_dir+'/templates/ext_model.pkl','rb'), encoding='latin1')
        Av=Av/A_1*A_2
        ...
        HBD = loadtxt(script_dir+'/templates/HBD_ext.txt')
        HBD[:,0]=HBD[:,0]/A_1*A_2
